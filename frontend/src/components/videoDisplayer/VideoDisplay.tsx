...
import { uploadVideoSegments, uploadMeetingVideoChunk } from "./api/save";
...
const Display: React.FC<DisplayProps> = ({
  ...
  const { updateVideoState, startMeeting } = useVideoSocketContext();
  ...
  const handleMouseUp = async (e: React.MouseEvent<HTMLDivElement>) => {
    if (draggedItem?.type === "edited-video") {
      ...
      try {
        ...
        await uploadMeetingVideoChunk(roomName!, file, 0);
      } catch (err) {
      ...
      try {
        ...
        console.error("Failed to upload segments");
      }
